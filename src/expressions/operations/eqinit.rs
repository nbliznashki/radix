                ...
                match (index_input_lhs, index_input_rhs, &bits_new){
                    (None, None, None)=>{
                        data_output.par_extend(
                            data_input_lhs.par_iter().zip_eq(data_input_rhs.par_iter()).map(|(lv, rv)| lv.eq(&T1::from(*rv)))
                        );
                    },
                    (Some(ind_lhs), None, None)=>{
                        data_output.par_extend(
                            ind_lhs.par_iter().zip_eq(data_input_rhs.par_iter()).map(|(li, rv)| (data_input_lhs[*li]).eq(&T1::from(*rv)))
                        );
                    },
                    (None, Some(ind_rhs), None)=>{
                        data_output.par_extend(
                            data_input_lhs.par_iter().zip_eq(ind_rhs.par_iter()).map(|(lv, ri)| (*lv).eq(&T1::from(data_input_lhs[*ri])))
                        );
                    },
                    (Some(ind_lhs), Some(ind_rhs), None)=>{
                        data_output.par_extend(
                            ind_lhs.par_iter().zip_eq(ind_rhs.par_iter()).map(|(li, ri)| (data_input_lhs[*li]).eq(&T1::from(data_input_lhs[*ri])))
                        );
                    },
